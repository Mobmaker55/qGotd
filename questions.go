...
func GetResponses(w http.ResponseWriter, r *http.Request) {
	...
	for i := range responses {
		...
		sendable = append(sendable, responses[i]) //literally just remove the ones that are hidden
	}
	var toSend []interface{}
...
func ReportResponse(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("Report sent by: "+GetUser(w, r).PreferredUsername+" on response", id)
	w.WriteHeader(http.StatusOK)
}
